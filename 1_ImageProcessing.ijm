...
//Loop for Every ROI and Threshold 
for(i=0; i<nROI; i++){
	print(i);
	selectImage(postZbandpass);
	run("Duplicate...", "duplicate");
...
saveAs("Tiff", path + File.separator +  "postZproject_Threshold.tif");
...
saveAs("Tiff", path + File.separator +  "PreZproject_threshold.tif");
run("Close All");
setBatchMode(false);
//Analyze Particlew
open(path + File.separator + "PreZproject_threshold.tif");
